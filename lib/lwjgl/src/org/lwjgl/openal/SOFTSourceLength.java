 ...
 * {@link SOFTBufferSamples AL_SOFT_buffer_samples} extension allows an core to query a buffer for its length in bytes, samples, and seconds, but
 * this is only for a single buffer; a buffer queue on a source still requires additional bookkeeping by the core to keep track of the length of
