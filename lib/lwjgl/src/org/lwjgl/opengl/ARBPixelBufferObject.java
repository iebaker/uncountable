 ...
 * <ul>
 * <li>Streaming texture updates: If the core uses {@link GL15#glMapBuffer MapBuffer}/{@link GL15#glUnmapBuffer UnmapBuffer} to write its data for glTexSubImage into a buffer object, at
 ...
 * <li>Asynchronous {@link GL11#glReadPixels ReadPixels}: If an core needs to read back a number of images and process them with the CPU, the existing GL interface
 ...
 * and then wait for all of the data to be available before returning control to the core. Then, the core can either process the data
 * immediately or call glReadPixels again; in neither case will the readback overlap with the processing. If the core issues several readbacks
 ...
 * <li>Render to vertex array: The core can use a fragment program to render some image into one of its buffers, then read this image out into a
