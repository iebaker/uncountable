...
import xyz.izaak.gamesystems.architecture.Basis;
...
public class Portal extends Quad {
    ...
    public Portal(String name, Vector3f basePosition, Vector3f normal, Vector3f up) {
        ...
        m_basePosition = Points.get(basePosition);
        m_normal = Points.get(normal);
        m_up = Points.get(up);
    }
    ...
    public Vector3f getBasePosition() {
        return Points.get(m_basePosition);
    }

    public Vector3f getNormal() {
        return Points.get(m_normal);
    }

    public Vector3f getUp() {
        return Points.get(m_up);
    }

    public Vector3f getLeft() {
        return Points.get(m_normal).cross(m_up);
    }
    ...
    public Basis getBackBasis() {
        return new Basis(Points.get(m_normal).negate(), m_up);
    }
    ...
    public boolean crossedBy(Vector3f startPosition, Vector3f endPosition) {
        Vector4f start = Points.homogeneousPoint(Points.get(startPosition));
        Vector4f ray = Points.homogeneousVector(Points.get(endPosition).sub(startPosition));
