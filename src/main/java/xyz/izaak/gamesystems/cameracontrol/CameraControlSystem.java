package xyz.izaak.gamesystems.cameracontrol;
...
import xyz.izaak.gamesystems.architecture.Seam;
import xyz.izaak.core.World;
...
public class CameraControlSystem extends GameSystem {
    ...
    private Vector3f m_goalVelocity = Points.get(Points.ORIGIN_3D);
    private Vector3f m_realVelocity = Points.get(Points.ORIGIN_3D);
    private Vector3f m_previousLocation = Points.get(Points.ORIGIN_3D);
    ...
    public void tick(float seconds) {
        ...
        m_realVelocity.add(Points.get(m_goalVelocity).sub(m_realVelocity).mul(m_accelerationFactor));
        camera.translate(Points.get(m_realVelocity).mul(seconds));
        checkPortals();
        m_goalVelocity = Points.get(Points.ORIGIN_3D);
    }
