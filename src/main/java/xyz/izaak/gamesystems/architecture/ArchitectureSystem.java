...
import xyz.izaak.gamesystems.architecture.setpieces.Quad;
...
public class ArchitectureSystem extends GameSystem {
    ...
    private static String m_modulesDirectoryName = "modules";
    private static String m_rulesDirectoryName = "rules";
    ...
    private void buildTemplateWalls(ModuleTemplate template, JSONArray d, JSONArray c) {
        ...
        Quad floor = new Quad();
        floor.setFillColor(Points.get(Points.WHITE));
        floor.setStrokeColor(Points.get(Points.WHITE));
        floor.scale(1.005f);
        ...
        ceiling.setFillColor(Points.get(Points.WHITE));
        ceiling.setStrokeColor(Points.get(Points.WHITE));
        ...
        rightWall.setFillColor(Points.get(Points.GRAY).mul(wallFactor));
        rightWall.setStrokeColor(Points.get(Points.WHITE));
        ...
        leftWall.setFillColor(Points.get(Points.GRAY).mul(wallFactor));
        leftWall.setStrokeColor(Points.get(Points.WHITE));
        ...
        farWall.setFillColor(Points.get(Points.GRAY).mul(wallFactor));
        farWall.setStrokeColor(Points.get(Points.WHITE));
        //farWall.scale(1.005f);
        farWall.translate(0.5f, 0.5f, 0.0f);
        farWall.scale(dimensions.x, dimensions.y, 1.0f);

        Quad nearWall = new Quad();
        nearWall.setFillColor(Points.get(Points.GRAY).mul(wallFactor));
        nearWall.setStrokeColor(Points.get(Points.WHITE));
    ...
    private void buildTemplatePortals(ModuleTemplate template, JSONArray portals) {
        for(int i = 0; i < portals.length(); ++i) {
            ...
            portal.setFillColor(Points.get(Points.BLACK));
            portal.setStrokeColor(Points.get(Points.WHITE));
